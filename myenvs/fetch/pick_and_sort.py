...
# MODEL_XML_PATH = os.path.join('pick_and_sort.xml')
MODEL_XML_PATH = os.path.join(os.path.dirname(__file__), 'assets', 'pick_and_sort.xml')
def goal_distance(goal_a, goal_b):
    assert goal_a.shape == goal_b.shape
    return np.linalg.norm(goal_a - goal_b, axis=-1)

class FetchPickAndSortEnv(robot_env.RobotEnv, gym.utils.EzPickle):
    def __init__(self, reward_type='sparse'):
        ...
        self.adapt_dict["field"] = [1.375, 0.75, 0.6, 0.36, 0.526, 0.2]
        self.adapt_dict["obstacles"] = [[1.375 + 0.175, 0.75 + 0, 0.6 - 0.150, 0.01, 0.526, 0.05],
                                        [1.375 + 0.35, 0.75 + 0, 0.6 - 0.15, 0.01, 0.526, 0.05],
                                        [1.375 + 0, 0.75 - 0.35, 0.6 - 0.15, 0.36, 0.01, 0.05],
                                        [1.375 + 0, 0.75 + 0.35, 0.6 - 0.15, 0.36, 0.01, 0.05],
                                        [1.375 + 0, 0.75 + 0.525, 0.6 - 0.15, 0.36, 0.01, 0.05],
                                        [1.375 + 0, 0.75 + -0.525, 0.6 - 0.15, 0.36, 0.01, 0.05],
                                        [1.375 + 0.2625, 0.75 + 0, 0.6 - 0.15, 0.0875, 0.01, 0.05],
                                        [1.375 + 0.2625, 0.75 + 0.175, 0.6 - 0.15, 0.0875, 0.01, 0.05],
                                        [1.375 + 0.2625, 0.75 - 0.175, 0.6 - 0.15, 0.0875, 0.01, 0.05],
    ...
    def _sample_goal(self):
        ...
        index = int(np.floor(self.np_random.uniform(0, 12)))
    ...
    def _env_setup(self, initial_qpos):
        ...
        self.target_9 = self.sim.data.get_site_xpos('target_9')
        self.target_10 = self.sim.data.get_site_xpos('target_10')
        self.target_11 = self.sim.data.get_site_xpos('target_11')
        self.target_12 = self.sim.data.get_site_xpos('target_12')


        self.targets = [self.target_1, self.target_2, self.target_3, self.target_4, self.target_5, self.target_6,
                        self.target_7, self.target_8, self.target_9, self.target_10, self.target_11, self.target_12]
    ...
    def render(self, mode='human', width=500, height=500):
        return super(FetchPickAndSortEnv, self).render(mode, width, height)
