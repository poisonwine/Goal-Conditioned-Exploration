...
MODEL_XML_PATH = os.path.join(os.path.dirname(__file__), 'assets', 'push_two_wall_obstacle.xml')
MODEL_XML_PATH2 = os.path.join(os.path.dirname(__file__), 'assets', 'push_one_wall_obstacle.xml')
MODEL_XML_PATH3 = os.path.join(os.path.dirname(__file__), 'assets', 'push_wall_heavy_obstacle_v2.xml')
MODEL_XML_PATH4 = os.path.join(os.path.dirname(__file__), 'assets', 'push_wall_heavy_obstacle_v5.xml')


class FetchPushWallObstacle_v1(fetch_env.FetchEnv, utils.EzPickle):
        ...
        if self.has_object:
            if self.random_box and self.np_random.uniform() < self.random_ratio:
                ...
                while (np.linalg.norm(object_xpos - self.initial_gripper_xpos[:2]) < 0.1 or
                       (object_xpos[0] - self.pos_wall[0] < self.size_wall[0] + self.size_object[0])):
                        object_xpos = self.initial_gripper_xpos[:2] + self.np_random.uniform(-self.obj_range, self.obj_range,size=2)
                        #object_xpos[1] = self.initial_gripper_xpos[1] + self.np_random.uniform(-self.obj_range, self.obj_range)
    ...
    def _sample_goal(self):
        ...
        if self.has_object:
            ...
            goal[0] = self.np_random.uniform(low=1.075+self.size_object[0], high=self.pos_wall[0]-self.size_object[0]-self.size_wall[0])
            goal[1] = self.np_random.uniform(low=0.5, high=1.0)
...
class FetchPushWallObstacleEnv_v2(fetch_env.FetchEnv, utils.EzPickle):
    ...
    def _reset_sim(self):
        ...
        if self.has_object:
            if self.random_box and self.np_random.uniform() < self.random_ratio:
                ...
                stick_xpos = object_xpos.copy()
                while (np.linalg.norm(object_xpos - self.initial_gripper_xpos[:2]) < 0.1
                       or (object_xpos[0] - self.pos_wall[0]) < self.size_wall[0] + self.size_object[0]):
    ...
    def _sample_goal(self):
        ...
        if self.has_object:
            goal = self.initial_gripper_xpos[:3] + self.target_offset + self.np_random.uniform(-self.target_range, self.target_range, size=3)
            goal[0] = self.np_random.uniform(low=1.075+self.size_object[0], high=self.pos_wall[0]-self.size_object[0]-self.size_wall[0])
            goal[1] = self.np_random.uniform(low=0.45, high=1)
            goal[2] = self.height_offset
            if self.target_in_the_air and self.np_random.uniform() < 0.5:
                goal[2] += self.np_random.uniform(0, 0.45)
        else:
...
class FetchPushWallObstacleEnv_v3(fetch_env.FetchEnv, utils.EzPickle):
    ...
    def _reset_sim(self):
        ...
        if self.has_object:
            ...
            stick_xpos = object_xpos.copy()
            while (np.linalg.norm(object_xpos - self.initial_gripper_xpos[:2]) < 0.1
                   or (object_xpos[0] - self.pos_wall[0]) < self.size_wall[0] + self.size_object[0] or (
                    stick_xpos[0] - self.pos_wall[0]) < self.size_wall[0] + self.size_obstacle[0]
                   or ((object_xpos[0] - stick_xpos[0]) < self.size_object[0] + self.size_obstacle[0] and abs(
                ...
                if self.np_random.uniform() > self.hard_case_ratio:
                    ...
                    stick_xpos[1] = 0.75 + self.np_random.uniform(-0.1, 0.1)
                    stick_xpos[0] = 1.3
    ...
    def _sample_goal(self):
        ...
        if self.has_object:
            goal = self.initial_gripper_xpos[:3] + self.target_offset + self.np_random.uniform(-self.target_range, self.target_range, size=3)
            goal[0] = self.np_random.uniform(low=1.075 + self.size_object[0],
                                             high=self.pos_wall[0] - self.size_object[0] - self.size_wall[0])
            goal[1] = self.np_random.uniform(low=0.5, high=1)
            goal[2] = self.height_offset
...
class FetchPushWallObstacleEnv_v4(fetch_env.FetchEnv, utils.EzPickle):
    ...
    def compute_reward_and_success(self, observation, goal, info):
        ...
        if self.reward_type == "dense":
            r = 0.1 * fetch_env.FetchEnv.compute_reward(self, achieved_goal, goal[0:3], info) + success
        else:
            r = fetch_env.FetchEnv.compute_reward(self, achieved_goal, goal[0:3], info)
        return r, success
