...
# MODEL_XML_PATH = os.path.join('pick_obstacle.xml')
MODEL_XML_PATH = os.path.join(os.path.dirname(__file__), 'assets', 'pick_obstacle.xml')
def goal_distance(goal_a, goal_b):
    assert goal_a.shape == goal_b.shape
    return np.linalg.norm(goal_a - goal_b, axis=-1)

class FetchPickObstacleEnv(robot_env.RobotEnv, gym.utils.EzPickle):
