...
# MODEL_XML_PATH = os.path.join('fetch', 'slide_new.xml')
MODEL_XML_PATH = os.path.join(os.path.dirname(__file__), 'assets', 'slide_new.xml')
def goal_distance(goal_a, goal_b):
    assert goal_a.shape == goal_b.shape
    return np.linalg.norm(goal_a - goal_b, axis=-1)

class FetchSlideNewEnv(robot_env.RobotEnv, gym.utils.EzPickle):
    def __init__(self, reward_type='sparse'):
        ...
        super(FetchSlideNewEnv, self).__init__(
    ...
    def render(self, mode='human', width=500, height=500):
        return super(FetchSlideNewEnv, self).render(mode, width, height)
