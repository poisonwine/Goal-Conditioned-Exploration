...
from myenvs.baxter import robot_env
...
class BaxterEnv(robot_env.RobotEnv):
    ...
    def __init__(
        ...
        distance_threshold, initial_qpos, reward_type, single_goal
    ):
    ...
    def _sample_goal(self):
        fixed_goal_position = np.array([ 0.55941889, 0.32,  -0.13329536])
        if self.has_object:
            goal = self.initial_gripper_xpos[:3] + self.np_random.uniform(-self.target_range, self.target_range, size=3)
            if self.single_goal:
                goal = fixed_goal_position # single goal fixed position
            goal += self.target_offset
            goal[2] = self.height_offset
            if self.target_in_the_air and self.np_random.uniform() < 0.5:
                goal[2] += self.np_random.uniform(0, 0.45)
            if self.target_in_the_air and self.single_goal:
                goal[2] = 0.16
                                        
            ## the kitting exps's target, but without use single goal
            if self.target_range < 0.08:
                goal = np.array([ 0.55941889, -0.29678608,  -0.13329536]) + self.np_random.uniform(-self.target_range, self.target_range, size=3)
        else:
            goal = self.initial_gripper_xpos[:3] + self.np_random.uniform(-0.15, 0.15, size=3)
            if self.single_goal:
                goal = fixed_goal_position
