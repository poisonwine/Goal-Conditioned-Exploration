...
from myenvs.maze.ant_maze.create_maze_env import create_maze_env
...
class AntMazeEnv(gym.GoalEnv):
  ...
  def get_obs(self):
    ...
    obs = self.maze._get_obs()
    ag = obs[self.goal_dims]
  ...
  def step(self, action):
    ...
    reward = self.compute_reward(s_xy, self.goal, None)
    info = {}
    self.num_steps += 1
    ...
    if self.num_steps >= self._max_episode_steps and not done:
      done = True
...
# class AntMazeGoalSetEnv(AntMazeEnv):
#   def __init__(self, variant='AntMaze-SR', eval=False):
#     super(AntMazeGoalSetEnv, self).__init__(variant=variant, eval=eval)
#
#   @property
#   def goal(self):
